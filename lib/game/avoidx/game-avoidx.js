...
.defines(function() {
	EntityGameAvoidX = ig.Entity.extend({
		...
		detectKeyDown:function(){
			if (!ig.game.pointer.isReleased) return;
			for (var index = 0; index < this.arrayEnemy.length; index++) {
				var enemy = this.arrayEnemy[index];
				if (enemy.status == 'hit') continue;

				var p1 = {
					x: ig.game.pointer.pos.x,
					y: ig.game.pointer.pos.y,
					width: 20,
					height: 20
				}
				var p2 = {
					x: enemy.pos.x - enemy.image.width * 0.5,
					y: enemy.pos.y - enemy.image.height * 0.5,
					width: enemy.image.width * enemy.sizeScale,
					height: enemy.image.height * enemy.sizeScale
				}
		...
		drawAvoided:function(){
			...
			var context = ig.system.context;
			var startX = this.marginBox + this.marginWidth * 0.5;
			var staryY = 80;

			var minutes = Math.floor(this.timeRemaining / 60);
			var seconds = Math.ceil(this.timeRemaining % 60);
			...
			context.font = 100 + "px spongeboy";
			context.fillText(string, startX, staryY + 120);
			context.restore();
		},
